...
<body style="background-color: #efeba0;">
	...
	<div class="srch">
		<form class="navbar-form" method="post" name="form1">
				...
				<input style="background-color: white; color: black;" class="form-control" type="text" name="search" placeholder="Admin Username" required="">				<!-- search input field  -->
	...
	<h2><center> REGISTERED ADMINS </center> </h2>
	<?php

		if(isset($_POST['submit']))
		{
			$q=mysqli_query($db,"SELECT id,firstname,lastname,username,email,contact FROM `admin` where username like '%$_POST[search]%' ");			/* query for search btn to search */

			if(mysqli_num_rows($q)==0)		/* data will not be found after searching */
			{
				echo "</br></br></br>";
				echo "<h2><b><i><center>";
				echo "Sorry..! No admin found with that user name. Try search again.";
				echo "</center></i></b></h2>";
			}
			else
			{
		...
		{
				$res=mysqli_query($db,"SELECT id,firstname,lastname,username,email,contact FROM `admin` ;");			/* created table */	
		...
		if (isset($_POST['submit1']))  
			{
				...
				{
					mysqli_query($db,"DELETE from `admin` where username = '$_POST[username]' ;"); 		/*		Delete query   */
